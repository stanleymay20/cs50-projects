...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        for x_word in self.domains[x]:
            # Check if there's *any* y_word such that x[i] == y[j]
            satisfies = any(
                x_word[i] == y_word[j] for y_word in self.domains[y] if x_word != y_word
            )
            if not satisfies:
        ...
        while queue:
            ...
            if self.revise(x, y):
                ...
                if not self.domains[x]:
                    return False
                # Add all arcs (z, x) where z is a neighbor of x but not y
    ...
    def select_unassigned_variable(self, assignment):
        ...
        return min(unassigned, key=mrv_degree)
